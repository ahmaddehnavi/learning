define("ace/mode/css", "require exports module ace/lib/oop ace/mode/text ace/tokenizer ace/mode/css_highlight_rules ace/mode/matching_brace_outdent ace/worker/worker_client ace/mode/folding/cstyle".split(" "), function (e, h) {
    var i = e("../lib/oop"), b = e("./text").Mode, a = e("../tokenizer").Tokenizer, c = e("./css_highlight_rules").CssHighlightRules, f = e("./matching_brace_outdent").MatchingBraceOutdent, d = e("../worker/worker_client").WorkerClient, l = e("./folding/cstyle").FoldMode, j = function () {
        this.$tokenizer = new a((new c).getRules(),
            "i");
        this.$outdent = new f;
    ...
    (function () {
        ...
        this.getNextLineIndent = function (f, a, d) {
            var b = this.$getIndent(a), f = this.$tokenizer.getLineTokens(a, f).tokens;
            return f.length && f[f.length - 1].type == "comment" ? b : (a.match(/^.*\{\s*$/) && (b = b + d), b)
        };
        this.checkOutdent = function (f, a, b) {
            return this.$outdent.checkOutdent(a, b)
        };
        this.autoOutdent = function (f, a, b) {
            this.$outdent.autoOutdent(a, b)
        };
        this.createWorker = function (f) {
            var a = new d(["ace"], "ace/mode/css_worker",
                "Worker");
            return a.attachToDocument(f.getDocument()), a.on("csslint", function (a) {
                var b = [];
                a.data.forEach(function (a) {
                    b.push({row: a.line - 1, column: a.col - 1, text: a.message, type: a.type, lint: a})
                });
                f.setAnnotations(b)
            }), a
        }
...
define("ace/mode/css_highlight_rules", "require exports module ace/lib/oop ace/lib/lang ace/mode/text_highlight_rules".split(" "), function (e, h) {
    var i = e("../lib/oop"), b = e("../lib/lang"), a = e("./text_highlight_rules").TextHighlightRules, c = function () {
        var a = [
        ...
        d.unshift({token: "paren.rparen", regex: "\\}", next: "start"});
        a = b.copyArray(a);
        ...
        var g = b.copyArray(c);
        g.unshift({token: "comment", regex: ".*?\\*\\/", next: "media"});
        c = b.copyArray(c);
        ...
        this.$rules = {start: [
            ...
            {token: "string", regex: ":[a-z0-9-_]+"},
            {token: "constant", regex: "[a-z0-9-_]+"}
        ], comment: e, ruleset: d, ruleset_comment: c, media_ruleset: a,
            media_comment: g}
    };
    i.inherits(c, a);
    h.CssHighlightRules = c
});
define("ace/mode/matching_brace_outdent", ["require", "exports", "module", "ace/range"], function (e, h) {
    var i = e("../range").Range, b = function () {
    };
    (function () {
        ...
        this.autoOutdent = function (a, b) {
            ...
            if (!f)return 0;
            var f = f[1].length, d = a.findMatchingBracket({row: b, column: f});
            if (!d || d.row == b)return 0;
            d = this.$getIndent(a.getLine(d.row));
            a.replace(new i(b, 0, b, f - 1), d)
        };
    ...
    h.MatchingBraceOutdent = b
});
define("ace/mode/folding/cstyle", "require exports module ace/lib/oop ace/range ace/mode/folding/fold_mode".split(" "), function (e, h) {
    var i = e("../../lib/oop"), b = e("../../range").Range, a = e("./fold_mode").FoldMode, c = h.FoldMode = function () {
    };
    i.inherits(c, a);
    (function () {
        ...
        this.getFoldWidgetRange = function (a, d, c) {
            var e = a.getLine(c), g = e.match(this.foldingStartMarker);
            if (g) {
                d = g.index;
                if (g[1])return this.openingBracketBlock(a,
                    g[1], c, d);
                a = a.getCommentFoldRange(c, d + g[0].length);
                return a.end.column = a.end.column - 2, a
            }
            if (d === "markbeginend")if (g = e.match(this.foldingStopMarker)) {
                d = g.index + g[0].length;
                if (g[2]) {
                    a = a.getCommentFoldRange(c, d);
                    return a.end.column = a.end.column - 2, a
                }
                c = {row: c, column: d};
                if (a = a.$findOpeningBracket(g[1], c))return a.column++, c.column--, b.fromPoints(a, c)
            }
        }
    }).call(c.prototype)
});
define("ace/mode/folding/fold_mode", ["require", "exports", "module", "ace/range"], function (e, h) {
    var i = e("../../range").Range;
    (function () {
        ...
        this.indentationBlock = function (b, a, c) {
            var e = /\S/, d = b.getLine(a), h = d.search(e);
            if (-1 != h) {
                for (var c =
                    c || d.length, j = b.getLength(), g = d = a; ++a < j;) {
                    var k = b.getLine(a).search(e);
                    if (-1 != k) {
                        if (k <= h)break;
                        g = a
                    }
                }
                if (g > d)return b = b.getLine(g).length, new i(d, c, g, b)
            }
        };
        this.openingBracketBlock = function (b, a, c, e, d) {
            c = {row: c, column: e + 1};
            if (a = b.$findClosingBracket(a, c, d))return d = b.foldWidgets[a.row], null == d && (d = this.getFoldWidget(b, a.row)), "start" == d && a.row > c.row && (a.row--, a.column = b.getLine(a.row).length), i.fromPoints(c, a)
        }
